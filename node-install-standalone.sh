...
URL="${BASE_URL}/${RESOLVED}/node-${RESOLVED}-${PLATFORM}-${ARCH}.${EXT}"
...
info "Installing Node.js, please wait…"
	...
	fetchNode "${URL}"
if [ "${EXT}" = zip ]; then
    unzip -q ${PREFIX}/node-${RESOLVED}-${PLATFORM}-${ARCH}.${EXT} -d node/
	mv -f ${PREFIX}/node/node-${RESOLVED}-${PLATFORM}-${ARCH}/* ${PREFIX}/node/bin
else
	tar xz${VERBOSE} -f "node-${RESOLVED}-${PLATFORM}-${ARCH}.${EXT}" -C ${PREFIX}/node
	mv -f ${PREFIX}/node/node-${RESOLVED}-${PLATFORM}-${ARCH}/* ${PREFIX}/node
fi
	rm -r ${PREFIX}/node/node-${RESOLVED}-${PLATFORM}-${ARCH}/
	complete "Done"
